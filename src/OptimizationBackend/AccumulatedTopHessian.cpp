  ...
  for (EFResidual *r : p->residualsAll) {
    ...
    if (mode == 1) {
      ...
      for (int i = 0; i < patternNum; i++) {
        ...
        float rtz = *(((float *)&r->res_toZeroF) + i);
        rtz += *((float *)(rJ->JIdx) + i) * Jp_delta_x;
        rtz += *((float *)(rJ->JIdx + 1) + i) * Jp_delta_y;
        rtz += *((float *)(rJ->JabF) + i) * delta_a;
        rtz += *((float *)(rJ->JabF + 1)) * delta_b;
...
template void AccumulatedTopHessianSSE::addPointC<0>(
    EFPoint *p, EnergyFunctional const *const ef, int tid);
template void AccumulatedTopHessianSSE::addPointC<1>(
    EFPoint *p, EnergyFunctional const *const ef, int tid);
template void AccumulatedTopHessianSSE::addPointC<2>(
    EFPoint *p, EnergyFunctional const *const ef, int tid);
template <int mode>
