...
#else
      float Jp_delta_x = Jp_delta_x_1;
      float Jp_delta_y = Jp_delta_y_1;
      float delta_a = dp[6];
      float delta_b = dp[7];
      for (int i = 0; i < patternNum; i++) {
        // PATTERN: E = (2*res_toZeroF + J*delta) * J*delta.
        float Jdelta = Jp_delta_x * rJ->JIdx[0][i];
        Jdelta += Jp_delta_y * rJ->JIdx[1][i];
        Jdelta += delta_a * rJ->JabF[0][i];
        Jdelta += delta_b * rJ->JabF[1][i];
        float r0 = r->res_toZeroF[i];
        r0 += r0;
        r0 += Jdelta;
        Jdelta = Jdelta * r0;

        E.updateSingleNoShift(Jdelta);
      }
#endif
