...
public:
  ...
  ImuError(const ImuMeasurementDeque &imuMeasurements,
           const ImuParameters &imuParameters, const Time &t_0,
           const Time &t_1);
   ...
   */
  static int propagation(const ImuMeasurementDeque &imuMeasurements,
                         const ImuParameters &imuParams, Transformation &T_WS,
                         SpeedAndBias &speedAndBiases, const Time &t_start,
                         const Time &t_end, covariance_t *covariance = 0,
  ...
  int redoPreintegration(const Transformation &T_WS,
                         const SpeedAndBias &speedAndBiases) const;
  ...
  std::string typeInfo() const { return "ImuError"; }

protected:
  // parameters
  ImuParameters imuParameters_; ///< The IMU parameters.

  // measurements
  ImuMeasurementDeque imuMeasurements_; ///< The IMU measurements used. Must be
                                        /// spanning t0_ - t1_.
  ...
  mutable std::mutex
      preintegrationMutex_; //< Protect access of intermediate results.
  ...
  mutable information_t
      information_; ///< The information matrix for this error term.
  mutable information_t squareRootInformation_; ///< The square root information
                                                /// matrix for this error term.
};
}
