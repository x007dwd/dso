...
ImuError::ImuError(const dsio::ImuMeasurementDeque &imuMeasurements,
                   const dsio::ImuParameters &imuParameters,
                   const dsio::Time &t_0, const dsio::Time &t_1) {
  setImuMeasurements(imuMeasurements);
  setImuParameters(imuParameters);
  setT0(t_0);
  setT1(t_1);

  assert(t_0 >= imuMeasurements.front().timeStamp);
  assert(t_1 <= imuMeasurements.back().timeStamp);
}

// Propagates pose, speeds and biases with given IMU measurements.
int ImuError::redoPreintegration(
    const Transformation &T_WS,
    const dsio::SpeedAndBias &speedAndBiases) const {
  ...
  int i = 0;
  for (ImuMeasurementDeque::const_iterator it = imuMeasurements_.begin();
    ...
    F_delta.block<3, 3>(0, 3) =
        -crossMx(acc_integral_ * dt + 0.25 * (C + C_1) * acc_S_true * dt * dt);
...
// Propagates pose, speeds and biases with given IMU measurements.
int ImuError::propagation(const ImuMeasurementDeque &imuMeasurements,
                          const ImuParameters &imuParams, Transformation &T_WS,
                          SpeedAndBias &speedAndBiases, const Time &t_start,
  ...
  for (ImuMeasurementDeque::const_iterator it = imuMeasurements.begin();
    ...
    if (covariance) {
      ...
      // transform
      F_delta.block<3, 3>(0, 3) =
          -crossMx(acc_integral * dt + 0.25 * (C + C_1) * acc_S_true * dt * dt);
      F_delta.block<3, 3>(0, 6) = Eigen::Matrix3d::Identity() * dt;
      F_delta.block<3, 3>(0, 9) =
          dt * dv_db_g +
          0.25 * dt * dt * (C * acc_S_x * cross + C_1 * acc_S_x * cross_1);
...
bool ImuError::EvaluateWithMinimalJacobians(double const *const *parameters,
  ...
  // get speed and bias
  SpeedAndBias speedAndBiases_0;
  SpeedAndBias speedAndBiases_1;
  ...
  // actual propagation output:
  {
    ...
    F0.block<3, 3>(0, 3) = C_S0_W * crossMx(delta_p_est_W);
    F0.block<3, 3>(0, 6) = C_S0_W * Eigen::Matrix3d::Identity() * Delta_t;
    F0.block<3, 3>(0, 9) = dp_db_g_;
    F0.block<3, 3>(0, 12) = -C_doubleintegral_;
    F0.block<3, 3>(3, 3) = (plus(Dq * T_WS_1.q().inverse()) * oplus(T_WS_0.q()))
                               .topLeftCorner<3, 3>();
    F0.block<3, 3>(3, 9) =
        (oplus(T_WS_1.q().inverse() * T_WS_0.q()) * oplus(Dq))
            .topLeftCorner<3, 3>() *
        (-dalpha_db_g_);
    F0.block<3, 3>(6, 3) = C_S0_W * crossMx(delta_v_est_W);
    ...
    F1.block<3, 3>(3, 3) =
        -(plus(Dq) * oplus(T_WS_0.q()) * plus(T_WS_1.q().inverse()))
             .topLeftCorner<3, 3>();
