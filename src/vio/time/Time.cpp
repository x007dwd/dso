...
#include <vio/time/implementation/Time.hpp>
 ...
 */
void dsio_walltime(uint32_t &sec, uint32_t &nsec) {
 ...
 */
int dsio_nanosleep(const uint32_t &sec, const uint32_t &nsec) {
#if defined(WIN32)
  HANDLE timer = NULL;
 ...
 */
bool dsio_wallsleep(uint32_t sec, uint32_t nsec) {
#if defined(WIN32)
  dsio_nanosleep(sec, nsec);
#else
  timespec req = {sec, nsec};
  timespec rem = {0, 0};
...
bool Time::waitForValid() { return waitForValid(dsio::WallDuration()); }

bool Time::waitForValid(const dsio::WallDuration & /*timeout*/) {
  /*dsio::WallTime start = ros::WallTime::now();
  while (!isValid() && !stopped_) {
    dsio::WallDuration(0.01).sleep();
    if (timeout > dsio::WallDuration(0, 0) && (dsio::WallTime::now() - start >
  timeout)) {
        return false;
    }
  }
...
bool Duration::sleep() const {
  if (Time::useSystemTime()) {
    return dsio_wallsleep(sec, nsec);
  } else {
    Time start = Time::now();
    Time end = start + *this;
    if (start.isZero()) {
...
bool WallDuration::sleep() const { return dsio_wallsleep(sec, nsec); }
