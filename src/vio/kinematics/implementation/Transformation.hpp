...
#include "vio/kinematics/operators.hpp"
namespace dsio {
...
inline Eigen::Vector3d Transformation::
operator*(const Eigen::Vector3d &rhs) const {
  return C_ * rhs;
}
inline Eigen::Vector4d Transformation::
...
inline bool
Transformation::oplus(const Eigen::MatrixBase<Derived_delta> &delta) {
...
inline bool Transformation::oplusJacobian(
  ...
  S(2, 2) = 0.5;
  const_cast<Eigen::MatrixBase<Derived_jacobian> &>(jacobian)
      .template bottomRightCorner<4, 3>() = dsio::oplus(q_) * S;
  return true;
}
...
inline bool Transformation::liftJacobian(
  ...
  const_cast<Eigen::MatrixBase<Derived_jacobian> &>(jacobian)
      ...
      2 * dsio::oplus(q_.inverse()).template topLeftCorner<3, 4>();
  return true;
}
}
