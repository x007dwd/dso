...
//void PangolinDSOViewer:: split(const std::string& src, const std::string& delim, std::vector<std::string>& dest)
//{
//    std::string str = src;
...
//	    setting_desiredImmatureDensity = settings_nCandidates.Get();
		...
		setting_desiredPointDensity = 3600;
		setting_desiredImmatureDensity = 2400;
...
//从fullsystem中调用，把新的image插入viewer
