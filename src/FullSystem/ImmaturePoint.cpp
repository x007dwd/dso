...
ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian *host_, float type,
  ...
  for (int idx = 0; idx < patternNum;
       idx++) // patternNum为8,提取包括目标点在内的8个pixel
  {
...
ImmaturePointStatus ImmaturePoint::traceleft(FrameHessian *frame, Mat33f K) {
  // KRKi为单位矩阵
...
ImmaturePointStatus ImmaturePoint::traceright(FrameHessian *frame, Mat33f K) {
...
ImmaturePoint::traceOn(FrameHessian *frame, Mat33f hostToFrame_KRKi,
  ...
  //check, be slightly more relaxed by this factor. 	const float
  //slackInterval = 0.8;			// if pixel-interval is smaller than this,
  //leave it be. 	const float minImprovementFactor = 2;		// if
