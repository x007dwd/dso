...
struct FrameHessian {
  ...
  inline ~FrameHessian() {
    ...
    if (dI_OriginalImage != 0)
      delete[] dI_OriginalImage;
    if (orgImg != 0)
      delete orgImg;
    if (debugImage != 0)
      delete debugImage;
...
struct CalibHessian {
  ...
  Vec2f getAffLL(float host_exposure, float target_exposure, AffLight host_g2l,
                 AffLight target_g2l);
};
...
struct PointHessian {
  ...
  bool colorOverexposed[MAX_RES_PER_POINT]; // host-weights for respective
                                            // residuals.
