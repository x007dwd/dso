...
// density是numwant, 返回一共select了多少个点
...
/*	if(setting_pixelSelectionUseFast>0 && allowFast)
	...
	else*/
	{
		...
		//根据recursionsLeft决定递归次数，即makemaps的次数（控制采样点的多少）
...
//======Candidate Point Selection, 返回一共select了多少个点======
		...
		float* map_out, int pot, float thFactor)
{
	...
	int n3=0, n2=0, n4=0;
	for(int y4=0;y4<h;y4+=(4*pot)) for(int x4=0;x4<w;x4+=(4*pot))  //pot设为2, x4,y4递增8
	{
		int my3 = std::min((4*pot), h-y4);
		int mx3 = std::min((4*pot), w-x4);
		int bestIdx4=-1; float bestVal4=0;
		Vec2f dir4 = directions[randomPattern[n2] & 0xF];   //取randomPattern[n2]的后四位
		for(int y3=0;y3<my3;y3+=(2*pot)) for(int x3=0;x3<mx3;x3+=(2*pot)) //在x4,y4划定的9*9框里操作, x3,y3递增4
		{
			int x34 = x3+x4;
			int y34 = y3+y4;
			...
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot)  //在x3,y3划定的5*5框里操作, x2,y2递增2
			{
				int x234 = x2+x34;
				int y234 = y2+y34;
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1)  //在x2,y2划定的3*3框里操作, 逐像素搜索
				{
					assert(x1+x234 < w);
					assert(y1+y234 < h);
