...
//	std::string firstDisparityPath = "/home/jiatianwu/dso/05/disparity/000000.png";
//	cv::Mat firstDisparity = cv::imread(firstDisparityPath);
...
/*	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		if(lvl == 0)
		{
			...
			// npts_right = sel.makeMaps(firstRightFrame, statusMap,densities[lvl]*w[0]*h[0],1,false,2);
		}
		else
		{
			npts = makePixelStatus(firstFrame->dIp[lvl], statusMapB, w[lvl], h[lvl], densities[lvl]*w[0]*h[0]);
			// npts_right = makePixelStatus(firstRightFrame->dIp[lvl], statusMapB, w[lvl], h[lvl], densities[lvl]*w[0]*h[0]);
		}
...
//					assert(patternNum==9);
					...
					float sgmDisparity = getMatInterpolatedElement11BiLin(firstDisparity, x, y);
					pl[nl].idepth = sgmDisparity/bf;
					pl[nl].iR = sgmDisparity/bf;
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
		  // 先操作第0层
			    ...
			    ImmaturePointStatus stat = pt->traceright(firstRightFrame, K);
