...
void CoarseTracker::makeCoarseDepthForFirstFrame(FrameHessian *fh) {
  ...
  memset(idepth[0], 0,
         sizeof(float) * w[0] * h[0]); // memset用于一段内存块中填充值
  ...
  for (PointHessian *ph : fh->pointHessians) {
    ...
    //        printf("idepth with weight is  %f original is %f \n",
    //        idepth[0][u+w[0]*v], ph->idepth);
  }
  ...
  /*    //对于H矩阵中的每一个frame
      for(FrameHessian* fh : frameHessians)
      {
          ...
          for(PointHessian* ph : fh->pointHessians)
          {
              if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second
     == ResState::IN) //contains information about residuals to the last two (!)
     frames. ([0] = latest, [1] = the one before).
              {
                  PointFrameResidual* r = ph->lastResiduals[0].first;
                  assert(r->efResidual->isActive() && r->target == lastRef);
                  // centerProjectedTo为Vec3f, 分别存u,v,
     new_idepth,代表的就是该帧上一帧能tracking到的点在这一帧的坐标和逆深度 int u
     = r->centerProjectedTo[0] + 0.5f;    //centerProjectedTo为Vec3f, 分别存u,v,
     new_idepth int v = r->centerProjectedTo[1] + 0.5f; float new_idepth =
     r->centerProjectedTo[2]; float weight = sqrtf(1e-3 /
     (ph->efPoint->HdiF+1e-12));
  ...
  for (int lvl = 1; lvl < pyrLevelsUsed; lvl++) {
    ...
    for (int y = 0; y < hl; y++)
      for (int x = 0; x < wl; x++) {
        ...
        //				printf("idpeth_l is %f weightSums_l is %f \n",
        //idepth_l[x + y*wl], weightSums_l[x + y*wl] );
      }
  }
  ...
  /*	int lvl=0;
          ...
          float* lpc_idepth = pc_idepth[lvl];
          float* lpc_color = pc_color[lvl];
          Eigen::Vector3f* dIRefl = fh->dIp[lvl];

    for(PointHessian* ph : fh->pointHessians)
    {
      lpc_u[lpc_n] = ph->u;
      lpc_v[lpc_n] = ph->v;
      lpc_color[lpc_n] = dIRefl[int(ph->u + ph->v * wl)][0];
      lpc_idepth[lpc_n] = ph->idepth;
      lpc_n++;
      // printf("u and v are: %d, %d\n", ph->u, ph->v);
    }
...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian *> frameHessians,
  ...
  for (FrameHessian *fh : frameHessians) {
    ...
    //		std::cout << fh->pointHessians.size() << std::endl;

    //对于frame中的每一个点
    for (PointHessian *ph : fh->pointHessians) {
      if (ph->lastResiduals[0].first != 0 &&
          ph->lastResiduals[0].second == ResState::IN) // contains information
                                                       // about residuals to the
                                                       // last two (!) frames.
                                                       // ([0] = latest, [1] =
                                                       // the one before).
      {
        ...
        assert(r->efResidual->isActive() && r->target == lastRef);
        // centerProjectedTo为Vec3f, 分别存u,v,
        // new_idepth,代表的就是该帧上一帧能tracking到的点在这一帧的坐标和逆深度
        int u = r->centerProjectedTo[0] +
                0.5f; // centerProjectedTo为Vec3f, 分别存u,v, new_idepth
        ...
        pt_track->idepth_min_stereo = 0;
        pt_track->idepth_max_stereo = NAN;
        ImmaturePointStatus pt_track_right = pt_track->traceright(fh_right, K1);
        ...
        if (pt_track_right == ImmaturePointStatus::IPS_GOOD) {
          ...
          pt_track_back->idepth_min_stereo = 0;
          pt_track_back->idepth_max_stereo = NAN;
          ImmaturePointStatus pt_track_left =
              pt_track_back->traceleft(fh_target, K1);
  ...
  for (int lvl = 0; lvl < 2; lvl++) {
    ...
    for (int it = 0; it < numIts; it++) {
      ...
      float *weightSumsl_bak = weightSums_bak[lvl];
      memcpy(weightSumsl_bak, weightSumsl, w[lvl] * h[lvl] * sizeof(float));
      float *idepthl = idepth[lvl]; // dont need to make a temp copy of depth,
                                    // since I only read values with
  ...
  for (int lvl = 2; lvl < pyrLevelsUsed; lvl++) {
    ...
    float *weightSumsl_bak = weightSums_bak[lvl];
    memcpy(weightSumsl_bak, weightSumsl, w[lvl] * h[lvl] * sizeof(float));
    float *idepthl = idepth[lvl]; // dotnt need to make a temp copy of depth,
                                  // since I only read values with
                                  // weightSumsl>0, and write ones with
                                  // weightSumsl<=0.
    for (int i = w[lvl]; i < wh; i++) {
...
Vec6 CoarseTracker::calcRes(int lvl, SE3 refToNew, AffLight aff_g2l,
  ...
  for (int i = 0; i < nl; i++) {
    ...
    if (lvl == 0 && i % 32 == 0) {
      ...
      //这一块是在算什么呢？把正负方向旋转平移及纯平移拿出来，算总和，意义是什么？
    ...
    if (!std::isfinite((float)hitColor[0]))
      continue;
    //跟lsd一样，affLL是affine transformation,
    //有a,b两个参数。hitColor是插值出来的三维向量
  ...
  Vec6 rs;
  rs[0] = E;           // energy，即为总残差的大小
  rs[1] = numTermsInE; //参与计算energy的残差数量。
