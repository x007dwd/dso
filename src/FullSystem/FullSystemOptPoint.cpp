...
FullSystem::optimizeImmaturePoint(ImmaturePoint *point, int minObs,
  ...
  for (int k = 0; k < setting_discreteSeachItsOnPointActivation; k++) {
    ...
    for (int i = 0; i < nres; i++) {
      float idPerPix =
          1.0f / point->getdPixdd(&Hcalib, residuals + i, currentIdepth);
      if (idPerPix < discreteStep)
        discreteStep = idPerPix;
    }

    if (!std::isfinite(discreteStep) || discreteStep < 1e-8)
      continue;

    for (float delta = -8 * discreteStep; delta <= 8 * discreteStep;
         delta += discreteStep * searchStep) {
      float E = 0;
      for (int i = 0; i < nres; i++) {
        E += point->calcResidual(&Hcalib, 1, residuals + i,
                                 currentIdepth + delta);
      }
      if (E < minE) {
        minE = E;
