...
Undistort *Undistort::getUndistorterForMsg(double *para) {

  float ic[10];
  Undistort *u;
  u = new UndistortOpenCV(para);
...
ImageAndExposure *Undistort::undistort(const MinimalImage<T> *image_raw,
                                       ...
                                       float factor) const {
  // std::cout << "here in undistort wwww" << std::endl;
  // std::cout << "kuaile  " << std::endl;
  // std::cout << image_raw->w << std::endl;
...
void Undistort::readFromVec(double *para, int nPars) {
  // std::cout << "in my read" << std::endl;
  photometricUndist = 0;
  valid = false;
  passthrough = false;
  remapX = 0;
  ...
  remapX = new float[wG[0] * hG[0]];
  remapY = new float[wG[0] * hG[0]];

  w = wOrg = wG[0];
  h = hOrg = hG[0];

  std::cout << "in undistort wOrg and hOrg" << wOrg << hOrg << std::endl;
  makeOptimalK_crop();

  std::cout << "bef  benchmark" << std::endl;
  ...
  std::cout << "check hw" << h << '\t' << w << '\t' << hG[0] << wG[0]
            << std::endl;
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) {
      remapX[x + y * w] = x;
  ...
  std::cout << "before distortCoordinate" << std::endl;
...
UndistortOpenCV::UndistortOpenCV(double *para) { readFromVec(para, 8); }
...
void UndistortOpenCV::distortCoordinates(float *in_x, float *in_y, float *out_x,
  ...
  for (int i = 0; i < n; i++) {
    ...
    //		float thetad = theta * (1 + k1 * theta2 + k2 * theta4 + k3 *
    // theta6
    //+
    // k4 * theta8);
